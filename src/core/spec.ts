...
// require alias
type NormalizedOptions = Omit<Options, 'alias'> &
  Required<Pick<Options, 'alias'>>;

function normalize(options: SpecOptions | undefined) {
  const opts: NormalizedOptions = { alias: Object.create(null) };
  if (!options) {
    return opts;
  }
...
function _spec(raw: string | null, _options: NormalizedOptions): Spec {
  const _args: { [arg: string]: Options | null | undefined } =
    Object.create(null);
  let _curr:
    | { arg: string; options: NormalizedOptions; spec?: Spec }
    | undefined;
  ...
  function assignArg(arg: string, options: NormalizedOptions) {
    if (arg in _args) {
